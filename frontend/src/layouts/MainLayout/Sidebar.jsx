...
import { Home, User, Code2, Briefcase, Clock, Mail, BookOpen } from "lucide-react";
...
// Scroll-spied against DOM section ids on the homepage.
...
// Routed, not scrolled to — leaves the single-page experience entirely.
const BLOG_NAV_ITEM = { id: "blog", label: "Blog", Icon: BookOpen };
const NAV_ITEMS = [...SCROLL_NAV_ITEMS, BLOG_NAV_ITEM];
...
export const Sidebar = () => {
  ...
  const scrollActiveId = useActiveSection(SCROLL_NAV_ITEMS.map((item) => item.id));
  const activeId = location.pathname.startsWith(ROUTES.BLOG) ? "blog" : scrollActiveId;

  // "blog" always routes. Everything else scrolls if already on the
  // homepage, or routes home first (carrying the target id along) if not —
  // scrollToSection is a no-op for an id with no matching DOM section, which
  // is exactly the case once /blog exists as a real separate route.
  ...
  return (
    <>
      ...
      <MobileTabBar activeId={activeId} onNavigate={handleNavigate} />
    </>
  );
};
