...
export const ScrollDissolve = ({ children, className = "", isFirst = false, isLast = false }) => {
  ...
  useEffect(() => {
    ...
    // A fixed settle timeout doesn't cover content that loads
    // asynchronously (e.g. a section whose children fetch from an API) —
    // if that takes longer than the timeout, fitScale stays locked against
    // whatever (wrong) height was measured before the real content arrived.
    // ResizeObserver reacts to the actual height change whenever it happens.
